...
export async function createEmailInvite(
  ...
  return runAction("createEmailInvite", async () => {
    ...
    if (existingUser && (await isMutuallyFollowing(session.user.id, existingUser.id))) {
      throw new UserError("You're already connected with this person");
    }
...
export async function createLinkInvite(): Promise<{ inviteId: string; url: string }> {
  return runAction("createLinkInvite", async () => {
    const session = await getSession();
    const now = new Date();
...
export async function listMyInvites(): Promise<PendingInviteItem[]> {
  return runAction("listMyInvites", async () => {
    const session = await getSession();
...
export async function getPendingIncomingInviteCount(): Promise<number> {
  return runAction("getPendingIncomingInviteCount", async () => {
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session) return 0;
...
export async function getInvitePreview(token: string): Promise<InvitePreview> {
  return runAction("getInvitePreview", async () => {
    const [row] = await db
      .select({
        id: invite.id,
        kind: invite.kind,
...
export async function acceptInvite(token: string): Promise<{ inviterId: string }> {
  return runAction("acceptInvite", async () => {
    ...
    if (row.kind === "email") {
      ...
      if (!acceptedCount) throw new UserError("This invite is no longer valid");
    } else {
